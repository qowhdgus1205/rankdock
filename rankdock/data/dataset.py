...
def read_smiles(data_path, smiles_column="SMILES"):
    import pandas as pd
    df = pd.read_csv(data_path)
    if smiles_column not in df.columns:
        raise ValueError(f"'{smiles_column}' column not found in {data_path}")
    return df[smiles_column].dropna().tolist()

class MoleculeDataset(Dataset):
    def __init__(self, smiles_list=None, data_path=None, smiles_column="SMILES", assume_valid=False):
        if smiles_list is None:
            if data_path is None:
                raise ValueError("Provide smiles_list or data_path")
            smiles_list = read_smiles(data_path, smiles_column=smiles_column)

        if assume_valid:
            self.valid_smiles = list(smiles_list)
        else:
            self.valid_smiles = []
    ...
    def __getitem__(self, index):
        ...
        for atom in mol.GetAtoms():
            ...
            if chiral_tag == Chem.rdchem.ChiralType.CHI_OTHER:
